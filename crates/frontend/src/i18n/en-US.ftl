...
progress-initiating-upload = Initiating upload...
...
e2e-item1 = Argon2id(password, salt) → 32-byte master key (19456 KiB memory, 2 iterations)
...
e2e-item4 = ChaCha20-Poly1305(encryption_key, derived_nonce, plaintext) → ciphertext
e2e-item5 = Files >64KB split into 64KB chunks, each encrypted with a unique derived nonce
e2e-item6 = Unique 16-byte salt and 12-byte base nonce per paste
e2e-item7 = Server never sees your password or plaintext
...
stats-item1 = Total pastes created (all-time / daily)
stats-item2 = Total successful decryptions (all-time / daily)
stats-item3 = Total failed password attempts (all-time / daily)
stats-item4 = Viewable at the homepage and
cli-download-link = Download pre-built binaries on GitHub
